...
function horsesFromConfig(config) {
  const emojis = Array.isArray(config.horse_emojis) && config.horse_emojis.length >= 4
    ...
    : ['­ƒÉÄ', 'ÔÜí', '­ƒî®´©Å', '­ƒîè'];
  return [
    { name: 'Tonnerre', emoji: emojis[0] || '­ƒÉÄ' },
    { name: '├ëclair', emoji: emojis[1] || 'ÔÜí' },
    { name: 'Foudre', emoji: emojis[2] || '­ƒî®´©Å' },
    { name: 'Temp├¬te', emoji: emojis[3] || '­ƒîè' },
  ];
}
...
function canUserPlay(state, member) {
  ...
  if (Array.isArray(state.config.allowed_role_ids) && state.config.allowed_role_ids.length > 0) {
    ...
    if (!allowed) return [false, 'Tu nÔÇÖas pas le r├┤le autoris├® pour jouer.'];
  }
  const debt = getUserDebt(state, member.id);
  if (debt > DEBT_LIMIT) {
    return [false, `Acc├¿s bloqu├®, dette actuelle ${debt.toLocaleString('fr-FR')} kamas.`];
  }
  return [true, null];
}
...
function buildMainMessageContent(state, timer = null) {
  ...
  let content = '**­ƒÅç ANIMATION GUILDE - Mise sur ta Dragodinde !**\n\n';
  content += 'Bienvenue dans **Mise sur ta Dragodinde !**\n\n';
  content += `­ƒÆ░ **Participation :** ${ENTRY_FEE.toLocaleString('fr-FR')} kamas\n`;
  content += `­ƒº¥ **Commission organisation :** ${COMMISSION.toLocaleString('fr-FR')} kamas par entr├®e\n`;
  content += `­ƒÅå **Gain joueurs :** ${REAL_BET.toLocaleString('fr-FR')} kamas par joueur humain engag├®\n`;
  content += `­ƒñû **Gain IA :** ${REAL_BET.toLocaleString('fr-FR')} kamas par IA battue\n`;
  content += `­ƒÜ½ **Blocage dette :** au-del├á de ${DEBT_LIMIT.toLocaleString('fr-FR')} kamas\n\n`;
  content += '**Comment jouer ?**\n';
  content += 'ÔÇó Clique sur **Participer**\n';
  content += 'ÔÇó Choisis le mode, puis le nombre dÔÇÖadversaires, puis ta dragodinde\n';
  content += 'ÔÇó En mode joueurs, les places manquantes sont compl├®t├®es par lÔÇÖIA ├á la fin du d├®lai\n\n';
  ...
  if (reservationIsActive(state) && !rt.waitingForPlayers && !rt.raceInProgress && !rt.cooldown) {
    ...
    content += `ÔÅ│ **R├®servation en cours** pour <@${rt.reservation.user_id}> pendant encore **${remaining} sec**\n\n`;
  }

  if (rt.iaPendingLaunch && rt.iaPendingUserId) {
    content += `­ƒñû **D├®part contre l'IA en pr├®paration** pour <@${rt.iaPendingUserId}>\n`;
    content += `­ƒÄ» IA pr├®vues : **${rt.iaPendingCount}**\n`;
    content += `ÔØî Annulation possible pendant **${IA_CANCEL_WINDOW_SECONDS} secondes**\n`;
    content += `­ƒÜÇ D├®part automatique au bout de **${IA_PRESTART_SECONDS} secondes**\n\n`;
  }
  ...
  if (rt.waitingForPlayers) {
    content += `­ƒæÑ **Recherche d'adversaires en cours** : ${rt.currentPlayers.length}/${rt.expectedHumans}\n`;
    content += `ÔÅ▒´©Å **D├®part dans :** ${getMatchmakingRemainingSeconds(state)} sec\n`;
    content += `­ƒöÆ **Inscriptions :** ${isJoinWindowLocked(state) ? 'ferm├®es' : 'ouvertes'}\n`;
    content += `Ôå®´©Å **Annulation possible :** ${canCancelParticipationNow(state) ? 'oui' : 'non'}\n\n`;
  }
  ...
  if (rt.currentPlayers.length) {
    ...
    for (const uid of rt.currentPlayers) {
      ...
      content += `ÔÇó <@${uid}> ${horse ? `${horse.emoji} ${horse.name}` : 'ÔØô'}\n`;
    }
  } else {
    content += 'Aucun participant pour lÔÇÖinstant.\n';
  }
...
function configRows(guild) {
  ...
  return [
    ...
    roleSelectRow('dragodinde:config:admin_role', 'R├┤le admin', roles),
    roleSelectRow('dragodinde:config:notif_role', 'R├┤le notification', roles),
    roleSelectRow('dragodinde:config:allowed_roles', 'R├┤les autoris├®s ├á jouer', roles, Math.min(roles.length || 1, 5)),
  ];
}
...
function humanHorseLines(state, userIds, horsesSnapshot = null) {
  ...
  return userIds.map((uid) => {
    ...
    const horse = horseIndex !== undefined ? horses[horseIndex] : null;
    return `${horse ? horse.emoji : 'ÔØô'} <@${uid}>${horse ? ` avec **${horse.name}**` : ''}`;
  }).join('\n') || 'ÔÇö';
}
...
function buildRaceStatusEmbed(state, phase, { creatorId = null, humans = [], aiCount = 0, pot = 0, winnerId = null, winnerName = null, horsesSnapshot = null, reason = null } = {}) {
  ...
  if (phase === 'waiting') {
    ...
    embed
      ...
      .addFields(
        { name: 'Joueurs engag├®s', value: humanHorseLines(state, humans, horsesSnapshot), inline: false },
        { name: 'Cagnotte actuelle', value: `${(REAL_BET * humans.length).toLocaleString('fr-FR')} kamas`, inline: false },
        { name: 'D├®part dans', value: `${remaining} sec`, inline: true },
        { name: 'Inscriptions', value: isJoinWindowLocked(state) ? 'Ferm├®es' : 'Ouvertes', inline: true },
        { name: 'D├®sistement', value: canCancelParticipationNow(state) ? 'Autoris├®' : 'Verrouill├®', inline: true },
      );
  }

  if (phase === 'launching') {
    embed.setDescription(`La course se pr├®pare avec **${humans.length}** joueur(s) et **${aiCount}** IA.\nCagnotte : **${pot.toLocaleString('fr-FR')} kamas**`)
  ...
  if (phase === 'cancelled') {
    embed.setDescription(reason || 'Course annul├®e.');
  }
...
function buildDashboardEmbed(state) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Joueurs endett├®s', value: String(indebtedPlayersCount(state)), inline: true },
...
async function createDebtRecord(interaction, state, horseIndex) {
  if (!state.config.logs_channel_id) {
    return { ok: false, reason: 'Salon de logs non configur├®.' };
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `**Cheval :** ${horses[horseIndex]?.emoji || '­ƒÉÄ'} ${horses[horseIndex]?.name || 'Inconnu'}\n` +
...
function generateTrack(state, positions, activeHorseIndexes) {
  ...
  for (const index of activeHorseIndexes) {
    ...
    const track = 'ÔöÇ'.repeat(progress) + '­ƒÅü' + 'ÔöÇ'.repeat(Math.max(0, TRACK_LENGTH - progress));
...
async function runCountdown(thread, seconds, label = 'D├®part dans') {
  const msg = await thread.send({ embeds: [new EmbedBuilder().setTitle('Pr├®-d├®part').setDescription(`${label} **${seconds}** secondes...`).setColor(0x3498DB).setTimestamp()] });
  for (let s = seconds - 1; s >= 1; s--) {
    await sleep(1000);
    await msg.edit({ embeds: [new EmbedBuilder().setTitle('Pr├®-d├®part').setDescription(`${label} **${s}** secondes...`).setColor(0x3498DB).setTimestamp()] }).catch(() => {});
  }
  await sleep(1000);
...
async function runRace(thread, state, contestants) {
  ...
  const animMsg = await thread.send({ content: `­ƒÅç **D├®part** ­ƒÅç\n${generateTrack(state, positions, activeHorseIndexes)}` });
  await sleep(1000);
...
async function createRaceThread(channel, prefix) {
  const starter = await channel.send({ content: '­ƒÅç' });
...
async function startIaRace(interaction, state, userId, channel, nbIa) {
  if (state.runtime.raceInProgress || state.runtime.waitingForPlayers || state.runtime.iaPendingLaunch) {
    await channel.send({ content: 'Une course est d├®j├á en cours.' }).catch(() => {});
    return;
  }
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      thread = made.thread;
      await thread.send({ embeds: [new EmbedBuilder().setTitle('Course contre lÔÇÖIA').setDescription(`Humain : ${humanHorseLines(live, [String(userId)], horsesSnapshot)}\nIA adverses : ${nbIa}\nCagnotte : ${totalPool.toLocaleString('fr-FR')} kamas`).setColor(0x3498DB).setTimestamp()] });
      ...
      if (winnerType === 'human') {
        await thread.send({ embeds: [new EmbedBuilder().setTitle('Victoire joueur').setDescription(`­ƒÅå <@${winnerId}> remporte la course avec ${horses[winnerHorseIdx].emoji} **${winnerName}** et gagne **${totalPool.toLocaleString('fr-FR')} kamas**.`).setColor(0x2ECC71).setTimestamp()] });
        ...
        await thread.send({ embeds: [new EmbedBuilder().setTitle('Victoire IA').setDescription(`­ƒñû LÔÇÖIA gagne avec ${horses[winnerHorseIdx].emoji} **${winnerName}**.`).setColor(0xE67E22).setTimestamp()] });
      ...
      await thread.send({ content: `Ce thread sera supprim├® dans ${THREAD_LIFETIME} secondes.` }).catch(() => {});
...
async function waitForPlayers(interaction, state, channel) {
  ...
  try {
    ...
    const horses = horsesFromConfig(live.config);
    if (winnerType === 'human') {
      await thread.send({ embeds: [new EmbedBuilder().setTitle('Victoire joueur').setDescription(`­ƒÅå <@${winnerId}> gagne avec ${horses[winnerHorseIdx].emoji} **${winnerName}** et remporte **${totalPool.toLocaleString('fr-FR')} kamas**.`).setColor(0x2ECC71).setTimestamp()] });
...
async function handleConfigSelect(interaction) {
  ...
  if (interaction.customId === 'dragodinde:config:logs_channel') {
    ...
    return interaction.reply({ content: `Salon des logs s├®lectionn├® : <#${interaction.values[0]}>`, ephemeral: true });
  }
  if (interaction.customId === 'dragodinde:config:dashboard_channel') {
    draft.dashboard_channel_id = interaction.values[0] || null;
    await maybeApplyDraftConfig(interaction, state, draft);
    return interaction.reply({ content: `Salon du dashboard s├®lectionn├® : <#${interaction.values[0]}>`, ephemeral: true });
  }
  if (interaction.customId === 'dragodinde:config:admin_role') {
    draft.admin_role_id = interaction.values[0] || null;
    await maybeApplyDraftConfig(interaction, state, draft);
    return interaction.reply({ content: `R├┤le admin : ${draft.admin_role_id ? `<@&${draft.admin_role_id}>` : 'Aucun'}`, ephemeral: true });
  }
  if (interaction.customId === 'dragodinde:config:notif_role') {
    draft.notification_role_id = interaction.values[0] || null;
    await maybeApplyDraftConfig(interaction, state, draft);
    return interaction.reply({ content: `R├┤le notification : ${draft.notification_role_id ? `<@&${draft.notification_role_id}>` : 'Aucun'}`, ephemeral: true });
  }
  if (interaction.customId === 'dragodinde:config:allowed_roles') {
    ...
    return interaction.reply({ content: `R├┤les autoris├®s : ${txt}`, ephemeral: true });
  }
...
function buildCommands() {
  return [
    new SlashCommandBuilder().setName('dragodinde_setup').setDescription('Configure le jeu et cr├®e lÔÇÖannonce ├®pingl├®e'),
...
async function handleChatInputCommand(interaction) {
  ...
  if (interaction.commandName === 'dragodinde_setup') {
    if (!isAdminMember(member, state)) return interaction.reply({ content: 'Tu dois ├¬tre administrateur pour utiliser cette commande.', ephemeral: true });
    if (state.config.main_message_id && state.config.main_channel_id) {
      return interaction.reply({ content: 'Un message de course existe d├®j├á. Utilise `/reset_total_dragodinde` pour repartir proprement.', ephemeral: true });
    }
    return interaction.reply({ content: 'Bienvenue dans la configuration du jeu. Choisis le salon des logs, le salon du dashboard, le r├┤le admin, le r├┤le de notification et les r├┤les autoris├®s ├á jouer :', components: configRows(guild), ephemeral: true });
  }
  ...
  if (interaction.commandName === 'set_emojis_dragodinde') {
    ...
    saveState(state);
    await interaction.reply({ content: `Emojis mis ├á jour : ${state.config.horse_emojis.join(' ')}`, ephemeral: true });
    await updateMainMessageByChannel(interaction.channel, state).catch(() => {});
    return true;
  }
  ...
  if (interaction.commandName === 'setup_dashboard_dragodinde') {
    ...
    return interaction.reply({ content: `Salon du dashboard d├®fini sur ${salon}.`, ephemeral: true });
  }
  ...
  if (interaction.commandName === 'debt_report_dragodinde') {
    ...
    const lines = debtRows.slice(0, 25).map(([uid, debt, betsCount, paymentsCount]) => `${debt > DEBT_LIMIT ? 'bloqu├®' : 'autoris├®'} <@${uid}>  **${debt.toLocaleString('fr-FR')} kamas** | paris: ${betsCount} | paiements: ${paymentsCount}`);
  ...
  if (interaction.commandName === 'reset_total_dragodinde') {
    ...
    return interaction.reply({ content: 'Reset total Dragodinde termin├®. Relance `/dragodinde_setup` pour repartir proprement.', ephemeral: true });
  }
...
async function handleButtonInteraction(interaction) {
  ...
  if (customId === 'dragodinde:join:main') {
    ...
    if (state.runtime.raceInProgress || state.runtime.iaPendingLaunch) return interaction.reply({ content: 'Une course est d├®j├á en cours ou en pr├®paration.', ephemeral: true });
    if (state.runtime.currentPlayers.includes(interaction.user.id)) return interaction.reply({ content: 'Tu es d├®j├á inscrit pour cette course.', ephemeral: true });

    if (state.runtime.waitingForPlayers) {
      if (state.runtime.currentPlayers.length >= state.runtime.expectedHumans) return interaction.reply({ content: 'Toutes les places sont d├®j├á prises.', ephemeral: true });
      if (isJoinWindowLocked(state)) return interaction.reply({ content: 'Les inscriptions sont ferm├®es durant les 30 derni├¿res secondes avant le d├®part.', ephemeral: true });
    ...
    if (reservationIsActive(state) && state.runtime.reservation.user_id !== interaction.user.id) {
      ...
      return interaction.reply({ content: `Une autre personne est en train de finaliser son inscription, priorit├® ├á <@${state.runtime.reservation.user_id}> pendant encore **${remaining} sec**.`, ephemeral: true });
    }
  ...
  if (customId.startsWith('dragodinde:mode:')) {
    ...
    if (interaction.user.id !== userId) return interaction.reply({ content: 'Pas autoris├®.', ephemeral: true });
    if (!reservationOwnedBy(state, userId, token)) return interaction.reply({ content: 'Cette tentative a expir├®. Recommence en cliquant sur Participer.', ephemeral: true });
    await interaction.update({ content: `Mode choisi : ${mode === 'ia' ? "Contre l'IA" : "Contre d'autres joueurs"}`, components: [] });
    const follow = await interaction.followUp({ content: mode === 'ia' ? 'Combien dÔÇÖIA veux-tu affronter ? (1, 2 ou 3)' : 'Combien dÔÇÖadversaires humains veux-tu ? (1, 2 ou 3)', components: countChoiceRows(userId, token, mode), ephemeral: true });
    return follow;
  }
  ...
  if (customId.startsWith('dragodinde:horse:')) {
    ...
    if (contextMode === 'new_match' && !reservationOwnedBy(state, userId, token)) {
      return interaction.reply({ content: 'Cette tentative a expir├®. Recommence en cliquant sur Participer.', ephemeral: true });
    }
    if (state.runtime.currentPlayers.includes(interaction.user.id)) return interaction.reply({ content: 'Tu es d├®j├á inscrit.', ephemeral: true });
    ...
    if (contextMode === 'join_waiting') {
      ...
      if (taken.has(horseIndex)) return interaction.reply({ content: 'Cette dragodinde est d├®j├á prise par un autre joueur.', ephemeral: true });
    }
    ...
    clearReservation(state);
    saveState(state);
    await interaction.followUp({ content: `Inscription valid├®e !`, ephemeral: true }).catch(() => {});
    await updateMainMessage(interaction.client, state).catch(() => {});

    if (selectedMode === 'ia') {
      await interaction.followUp({ content: `D├®part contre lÔÇÖIA dans **${IA_PRESTART_SECONDS} secondes**.`, components: cancelIaLaunchRows(userId), ephemeral: true }).catch(() => {});
      await startIaRace(interaction, state, userId, interaction.channel, Number(selectedCount || 1));
    } else {
      await interaction.followUp({ content: `Recherche dÔÇÖadversaires lanc├®e.`, components: cancelParticipationRows(userId), ephemeral: true }).catch(() => {});
  ...
  if (customId.startsWith('dragodinde:cancelia:')) {
    ...
    if (interaction.user.id !== userId) return interaction.reply({ content: 'Pas autoris├®.', ephemeral: true });
    if (!state.runtime.iaPendingLaunch || state.runtime.iaPendingUserId !== userId) return interaction.reply({ content: 'Il nÔÇÖy a plus de d├®part IA en attente.', ephemeral: true });
  ...
  if (customId.startsWith('dragodinde:cancel:')) {
    const [, , userId] = customId.split(':');
    if (interaction.user.id !== userId) return interaction.reply({ content: 'Pas autoris├®.', ephemeral: true });
    if (state.runtime.raceInProgress) return interaction.reply({ content: 'La course a d├®j├á commenc├®.', ephemeral: true });
    if (!state.runtime.waitingForPlayers) return interaction.reply({ content: 'Il nÔÇÖy a plus de phase dÔÇÖattente.', ephemeral: true });
    if (!state.runtime.currentPlayers.includes(userId)) return interaction.reply({ content: 'Tu nÔÇÖes plus inscrit ├á cette course.', ephemeral: true });
    if (!canCancelParticipationNow(state)) return interaction.reply({ content: `Le d├®sistement nÔÇÖest autoris├® que pendant les ${MATCH_CANCEL_WINDOW_SECONDS} premi├¿res secondes.`, ephemeral: true });
    ...
    await interaction.update({ content: 'Ta participation a ├®t├® annul├®e et ta dette retir├®e.', components: [] });
    await updateMainMessage(interaction.client, state).catch(() => {});
    await upsertRaceAnnouncement(interaction.client, state, interaction.channel, buildRaceStatusEmbed(state, 'waiting', { creatorId: state.runtime.currentMatchCreatorId, humans: [...state.runtime.currentPlayers], pot: REAL_BET * state.runtime.currentPlayers.length, horsesSnapshot: { ...state.runtime.playerHorses } })).catch(() => {});
    return true;
  }
  ...
  if (customId.startsWith('dragodinde:debtpay:')) {
    ...
    if (!isAdminMember(interaction.member, state)) return interaction.reply({ content: 'R├┤le admin requis.', ephemeral: true });
    if (record.status === 'paid') return interaction.reply({ content: 'Ce paiement est d├®j├á valid├®.', ephemeral: true });
    if (record.status === 'cancelled') return interaction.reply({ content: 'Cet engagement a d├®j├á ├®t├® annul├®.', ephemeral: true });
